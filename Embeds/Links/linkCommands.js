...
module.exports = async (client, id = []) => {
    ...
    channel.messages.fetch().then((messages) => {
        ...
        if (niraMessages.size === 0) {
            ...
            channel.messages.fetch({ around: "776144364438028288", limit: 1 }).then((msg) => {
                const links1Msg = msg.first();
                links1Msg.edit(links1);
            });
            channel.messages.fetch({ around: "776144365738524673", limit: 1 }).then((msg) => {
                const links2Msg = msg.first();
                links2Msg.edit(links2);
            });
            channel.messages.fetch({ around: "776144366665728041", limit: 1 }).then((msg) => {
