    ...
    if (userLimit === null && user !== undefined) {
      ...
      if (isDmChannel(message) || isInChannel(message, allChannels.fishy) || !isHomeGuild(message)) {
        if (canFish === true) {
          ...
          const threshold = (crypto.randomInt(0, 100000000000000) / 100000000000000) * total;
          ...
          const index = Math.floor((crypto.randomInt(0, 100000000000000) / 100000000000000) * group.puns.length);
          ...
          if (group.type === 'totalTrash') {
            ...
            var amount = Math.floor((crypto.randomInt(0, 100000000000000) / 100000000000000) * (group.max - group.min) + group.min);
