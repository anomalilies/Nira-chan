...
import { zoneChannels, allowlists, contributorRoleNames } from '../config/config.json';
...
export default async function (client: CommandoClient, partialReaction: MessageReaction) {
  const hallOfFame = <TextChannel>client.channels.cache.find((channel) => channel.id === zoneChannels.halloffame);
