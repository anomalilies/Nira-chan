...
import { emojis, zoneChannels, roles, contributorRoleNames } from '../config/config.json';
...
const handleServerBoosterRole = async (client: CommandoClient, oldMember: GuildMember, newMember: GuildMember) => {
  if (!oldMember.roles.cache.has(roles.serverBoosters) && newMember.roles.cache.has(roles.serverBoosters)) {
    const channel = <TextChannel>client.channels.cache.get(zoneChannels.general);

    if (channel == undefined) {
      return console.error("Couldn't find General Channel with ID", zoneChannels.general);
    }
