...
function getSimpleEmbed(title, description) {
    return new MessageEmbed().setColor(15849719).setTitle(title).setDescription(description);
}
...
module.exports = class InviteCommand extends Commando.Command {
    ...
    async run(message) {
        const embed = getSimpleEmbed("Loading...", "<a:loading:791389606616236052>");
        ...
        message.channel.send(embed).then(async (msg) => {
            ...
            if (msg.channel.type !== "dm" && msg.guild.id === homeguild) {
                ...
                msg.edit(newEmbed).then(msg.react(`${nirahello}`).then(msg.react(`${niracute}`)));

                const filter = (reaction, user) =>
                    [`${nirahello}`, `${niracute}`].includes(reaction.emoji.id) && user.id === message.author.id;
                const reactions = msg
                    ...
                    .then((reactions) => {
                        if (reactions.has(`${nirahello}`)) {
                            msg.edit(botInvite).then(msg.reactions.removeAll());
                        } else if (reactions.has(`${niracute}`)) {
                            msg.channel.send("https://discord.gg/zutomayo").then(msg.reactions.removeAll());
                        }
                    });
            } else {
                setTimeout(() => {
